...
def main(args):
    ...
    for epoch_num in range(args.max_epoch):
        ...
        for idx, data in enumerate(data_loader_train):
            ...
            label_points_all = torch.where(seg[0,0,:,:,:]==organ_id)
            min_x = min(label_points_all[0])
            min_y = min(label_points_all[1])
            min_z = min(label_points_all[2])
            max_x = max(label_points_all[0])
            max_y = max(label_points_all[1])
            max_z = max(label_points_all[2])

            print("corner points:",[min_x, min_y, min_z], [max_x, max_y, max_z])
            ...
            bceloss = bce_loss(masks.clip(0,1),seg)
            # masks[masks>0]=1
            # masks[masks<0]=0
            diceloss = dice_loss(masks.clip(0,1), seg)
